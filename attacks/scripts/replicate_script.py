...
SHARED_DIR = os.getenv("SHARED_DIR", "/shared_data")
...
PROCESSED_DIR = f"{SHARED_DIR}/processed"
...
def replicate_authentication(direction):
    ...
    try:
        ...
        gateway_fqdn = intercepted_parameters.get("gateway").get("gateway_fqdn")
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(15)
            try:
                logger.info(f"Conectando a {gateway_fqdn}:{gateway_port}")
                s.connect((gateway_fqdn, gateway_port))
                last_response = None
...
def replicate_send_metrics():
    ...
    try:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            s.settimeout(10)  # Timeout de 10 segundos
            gateway_fqdn = intercepted_parameters.get("gateway").get("gateway_fqdn")
            logger.info(f"Conectando al Gateway en {gateway_fqdn}:{gateway_port}")
            s.connect((gateway_fqdn, gateway_port))
